...
const NeedsStep = ({ needs, setNeeds, nextStep }) => {
  ...
  const continueNextStep = async () => {
    ...
    try {
      let response = await accountSetupRequest({ needs }, "needsStep");
  ...
  return (
    <div className="bg-black text-white space-y-4 p-4 sm:p-6 lg:p-8">
      <Logo />
      <h2 className="text-xl font-bold mb-3">Select your needs</h2>
      <p className="mb-5">
        Some barbers may offer your specific needs such as beard grooming,
        braiding etc.
      </p>
      <div className="h-[300px] overflow-y-auto">
            <div className="grid grid-cols-2 gap-4 mb-5">
                ...
                <button
                    ...
                    className={`${
                    needs.includes(need) ? "bg-barber-blue text-white" : "bg-[#0D0D0D] text-white"
